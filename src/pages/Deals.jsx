...
export default function Deals() {
  ...
  const filtered = ALL_DEALS.filter((d) => {
    const matchSearch =
      !searchValue ||
      d.destination.toLowerCase().includes(searchValue) ||
      d.country.toLowerCase().includes(searchValue);
  ...
  const savings = filtered.reduce(
    (sum, d) => sum + ((d.original || 0) - (d.discounted || 0)),
    0
  );
      ...
      >
        <div className="relative z-10 max-w-7xl mx-auto">
          ...
          <p className="text-white/70 mb-8">
            Hand-picked discounts on flights, hotels, and packages.
          </p>
      ...
      >
        <div className="max-w-7xl mx-auto flex items-center gap-3">
          ...
          >
            ...
            <strong> ${savings.toLocaleString()}</strong> across{" "}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filtered.map((deal) => {
            ...
            const pct = Math.round(
              (1 - deal.discounted / deal.original) * 100
            );

            return (
              <div
                key={deal.id}
                className="rounded-3xl overflow-hidden card-glass group hover:shadow-xl transition-all duration-300"
              >
